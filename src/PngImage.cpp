...
PngImage::PngImage(unsigned width, unsigned height, std::vector<std::uint8_t> pixels)
...
PngImage PngImage::load(const std::filesystem::path& path) {
    ...
    if (!file) throw std::runtime_error("Could not open PNG: " + path.string());
    ...
    for (unsigned y = 0; y < height; ++y) rows[y] = pixels.data() + static_cast<std::size_t>(y) * width * 4;
...
void PngImage::save(const std::filesystem::path& path) const {
    ...
    if (!file) throw std::runtime_error("Could not create PNG: " + path.string());
    ...
    for (unsigned y = 0; y < m_height; ++y) rows[y] = const_cast<png_bytep>(m_pixels.data() + static_cast<std::size_t>(y) * m_width * 4);
